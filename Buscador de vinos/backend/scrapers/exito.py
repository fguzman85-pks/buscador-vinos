"""Scraper for Éxito (exito.com) using their internal search API."""
import logging
import httpx
...
async def scrape_exito(query: Optional[str] = None) -> list[dict]:
    """Fetch wine products from Éxito using their VTEX search API."""
    results = []
    try:
        ...
        async with httpx.AsyncClient(headers=headers, timeout=15, follow_redirects=True) as client:
            ...
            for item in data[:20]:
                try:
                    ...
                    link = f"https://www.exito.com/{item.get('linkText', '')}/p"
                    ...
                    logger.debug(f"Error parsing Éxito item: {e}")
                    continue

    except Exception as e:
        logger.warning(f"Éxito scraper error: {e}")
