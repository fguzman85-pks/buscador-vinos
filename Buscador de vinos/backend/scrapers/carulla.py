"""Scraper for Carulla (carulla.com) using their VTEX search API."""
import logging
import httpx
...
STORE_NAME = "Carulla"
STORE_COLOR = "#2E8B57"
API_URL = "https://www.carulla.com/api/catalog_system/pub/products/search"


async def scrape_carulla(query: Optional[str] = None) -> list[dict]:
    """Fetch wine products from Carulla using their VTEX search API."""
    results = []
    try:
        ...
        async with httpx.AsyncClient(headers=headers, timeout=15, follow_redirects=True) as client:
            ...
            for item in data[:20]:
                try:
                    ...
                    link = f"https://www.carulla.com/{item.get('linkText', '')}/p"
                    ...
                    logger.debug(f"Error parsing Carulla item: {e}")
                    continue

    except Exception as e:
        logger.warning(f"Carulla scraper error: {e}")
